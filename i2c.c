...
void i2c_init(void) {  //initiating I2C setting bit rate to 100Khz
...
inline void i2c_stop() {   
	...
	while (TWCR & (1<<TWSTO)); //bus realeased
}
...
while ((TWCR & (1<<TWINT))==0); //kames sure that PKt has been sent
}
...
//Send/write just one byte
 void i2c_xmit_byte(uint8_t data) {
    ...
    TWCR = (1<<TWINT)|(1<<TWEN);
    while ((TWCR & (1<<TWINT)) == 0);    //This indicates that the DATA has been transmitted, and ACK/NACK has been received.
}
...
void eeprom_write_page(uint8_t addr, uint8_t *data) {
	//tkae addr where you statrt writng ,then send ptr 
	//vg
}
